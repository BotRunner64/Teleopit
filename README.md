...
│   ├── ingest_motion.py    # 统一数据入口：BVH/PKL/NPZ -> NPZ clips + manifest
│   └── data/               # 数据系统工具脚本
│       ├── migrate_legacy_dataset.py
│       ├── validate_dataset.py
│       └── build_dataset.py
├── teleopit/                # 核心源码
...
│   ├── configs/             # 数据集配置
│   ├── data/                # 训练数据系统核心逻辑 (manifest/validate/build)
...
推荐使用“manifest + validate + build”数据系统管理训练数据，保持可复现并支持持续 scale up（训练入口仍然使用 `--motion_file` 单文件）。
...
# 2. 统一 ingestion（推荐）
#    自动处理 BVH/PKL/NPZ -> data/motion/npz_clips/<source>/...，并自动追加 manifest
...
# 3. 若 clip_id 已存在，默认报错；显式允许更新用 --allow_update
# python scripts/ingest_motion.py ... --allow_update
...
# 混合 fps 数据可加 --target_fps（例如 30）后合并
...
- [数据系统](docs/dataset.md) — manifest、校验、构建、scale up 流程
