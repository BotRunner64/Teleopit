...
- Optional large-model training task: `Tracking-Flat-G1-VelCmdHistoryLarge`
...
- Teacher-student training and older legacy task variants are removed; `VelCmdHistoryAdaptive` remains supported as the adaptive-sampling variant
...
│   ├── registry.py       # Registers VelCmdHistory, VelCmdHistoryAdaptive, VelCmdHistoryRegular, VelCmdHistoryLarge, and MotionTrackingDeploy tasks
│   ├── env.py            # VelCmdHistory / VelCmdHistoryAdaptive / VelCmdHistoryRegular / VelCmdHistoryLarge / MotionTrackingDeploy env builders
│   └── rl.py             # TemporalCNN PPO cfg variants + deploy MLP PPO cfg
├── tasks/tracking/rl/
...
### Training Task
Supported training tasks are `Tracking-Flat-G1-VelCmdHistory`, `Tracking-Flat-G1-VelCmdHistoryAdaptive`, `Tracking-Flat-G1-VelCmdHistoryRegular`, `Tracking-Flat-G1-VelCmdHistoryLarge`, and `Tracking-Flat-G1-MotionTrackingDeploy`.
...
- VelCmdHistoryLarge keeps the same TemporalCNN observation/action path and environment as VelCmdHistory, but uses a wider/deeper TemporalCNN actor/critic
...
- VelCmdHistoryLarge also uses `window_steps=[0]` with `sampling_mode="uniform"`
