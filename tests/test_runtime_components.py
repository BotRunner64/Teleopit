...
def _make_runner(obs_builder: object) -> PolicyStepRunner:
...
def test_prepare_motion_command_velcmd_preserves_reference_root_xy(monkeypatch) -> None:
    ...
    np.testing.assert_allclose(prep.qpos[3:7], np.array([1.0, 0.0, 0.0, 0.0]), atol=1e-6)
